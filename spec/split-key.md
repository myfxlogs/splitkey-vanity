...
1. **Order check**: the derived `addr` MUST satisfy `--expect-pattern`
   (matching semantics per the §3.1 grammar for the parsed
   `(type, value)` tuple). Failure → `order mismatch` (seller
...
Generated by an independent implementation (Rust k256) and
